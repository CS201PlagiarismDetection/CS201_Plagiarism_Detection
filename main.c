// CS201 Plagiarism Detection Project
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <ctype.h>
...
struct TrieNode* createNode() {
    ...
    for (int i = 0; i < 27; i++) { // 26 letters + 1 for '|'
...
void insertPhrase(struct TrieNode* root, const char* phrase) {
    ...
    while (*phrase) {
        ...
        if (index >= 0 && index <= 26) { // Allow 'a-z' and '|'
...
void findCommonNgrams(struct TrieNode* root1, struct TrieNode* root2, char* currentNgram, int level, int* commonCount, char commonNgrams[MAX_COMMON_NGRAMS][MAX_WORD_LENGTH]) {
    ...
    for (int i = 0; i < 27; i++) { // Loop through all 26 letters and '|'
...
void countWordOccurrencesInFile(char* filename, char distinctWords[MAX_DISTINCT_WORDS][MAX_WORD_LENGTH], int distinctWordCount, int wordCounts[MAX_DISTINCT_WORDS]) {
    FILE* file = fopen(filename, "r");
    if (!file) {
        printf("Error: Could not open file %s\n", filename);
        return;
...
void compareTwoFiles(char* filename1, char* filename2, int minN, int maxN) {
    ...
    countWordOccurrencesInFile(filename1, distinctWords, distinctWordCount, wordCounts1);
    countWordOccurrencesInFile(filename2, distinctWords, distinctWordCount, wordCounts2);
